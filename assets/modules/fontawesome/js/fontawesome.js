...
(function() {
  ...
  var makeIconMasking = function(_ref) {
    ...
    children.push(defs, {
      tag: 'rect',
      attributes: _extends(
        { fill: 'currentColor', 'clip-path': 'url(#' + clipId + ')', mask: 'url(#' + maskId + ')' },
        ALL_SPACE,
      ),
    });
  ...
  var mutators = {
    replace: function replace(mutation) {
      ...
      if (node.parentNode && node.outerHTML) {
        node.outerHTML =
          newOuterHTML +
          (config.keepOriginalSource && node.tagName.toLowerCase() !== 'svg' ? '<!-- ' + node.outerHTML + ' -->' : '');
      ...
      toArray(root.querySelectorAll('*'))
        .filter(function(n) {
          return (
            ...
            !~TAGNAMES_TO_SKIP_FOR_PSEUDOELEMENTS.indexOf(n.tagName.toUpperCase()) &&
  ...
  var autoReplace = function autoReplace() {
    ...
    if (Object.keys(namespace.styles).length > 0 && IS_DOM && config.autoReplaceSvg)
